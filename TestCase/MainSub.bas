...
Sub Sakusei()
    ...
    Dim LastCol As Long: LastCol = GetLastColumn(RowKa)
    Dim CaseCnt As Long: CaseCnt = LastCol - COL_TEST_CASE + 1
    If CaseCnt < 1 Then Exit Sub
    ' テストケース連番振り直し
    Call SB_TestCase_CaseNo(RowKa, LastCol)
    ' 入力用配列
    CaseRange = Range(Cells(RowKa, COL_JOUKEN_KITAICHI), Cells(LastRow, LastCol))
    ' 出力用配列
    Erase CaseArr
...
Private Function SB_Kitaichi_LastRow(ByVal RowKi As Long) As Long
    ...
    Dim RowUR As Long: RowUR = ActiveWorkbook.ActiveSheet.UsedRange.Rows.Count
